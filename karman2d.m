% file = casc2sim.m
% purpose: simulate super broad band processes by cascading of "octaves"
%           in 2D!
function sim2D = karman2d(xs_ext,ys_ext,nu,Lx)
% Input
% xs_ext: x-values for which to extract the stochastic field
% ys_ext: y-values ...
% nu: Von Karman shape factor
% L: Maximum scale length included in stochastic field
% OUTPUT
% sim2D: simulated field, 
%Sorry: x = vertical and y horizontal!!!!!!!
...
% next; wavenumbers with dk2 = dk/4;
   ...
   xs = xs*Nx/8;
    ...
    %  rho_inty(:,iy-iy1+1) = interp1(xs,rho_coarse(:,iy),xs0);
      rho_inty = interp1(xs(ix1:ix2),rho_coarse(ix1:ix2,iy1:iy2),xs0,'cubic');
    %end
    
    %for ix=1:Nx
      rho_intxy = interp1(ys(iy1:iy2),rho_inty',ys0,'cubic')';
      rho1 = rho1 + rho_intxy;
...
stdsim2D=std(rho1(:));
sim2D = rho1(1:Nx_ext,1:Ny_ext)/stdsim2D;
